...
UA={'User-Agent':'Mozilla/5.0 VenuHybridCloud/1.0'}
...
def prices():
    ...
    for i,t in enumerate(dict.fromkeys(TICKERS+['QQQ'])):
        try:
            data[t]=fetch_one(t); print(f'[{i+1}] OK {t}')
        except Exception as e:
            fails[t]=str(e); print(f'[{i+1}] FAIL {t}: {e}')
        time.sleep(.08)
...
def cs_pct(df):
    return df.rank(axis=1,pct=True).clip(0,1)
...
def build_scores(px):
    ...
    spy63=m63['SPY']; spy126=m126['SPY']
    rel3=m63.sub(spy63,axis=0); rel6=m126.sub(spy126,axis=0)
    ...
    # Price-only baseline: quality/valuation neutral until point-in-time SEC/13F layer is added.
    score=(12.5+10+trend+contra+risk).clip(0,100).shift(1)
    return score,vol.shift(1)

def target(srow,vrow,entry=80,maxpos=20):
    e=srow[srow>=entry].sort_values(ascending=False).head(maxpos)
    w=pd.Series(0.0,index=srow.index)
    for t,s in e.items():
        base=.04 if s>=95 else .03 if s>=90 else .02 if s>=85 else .01
...
def simulate(px,scores,vol,start,end,entry,maxpos):
    ...
    cur=pd.Series(0.0,index=px.columns); prev=pd.Series(0.0,index=px.columns); out=[]; cost=.0013
...
def perf(r):
    ...
    if len(r)<2: return {}
    vol=r.std()*np.sqrt(252); dd=eq/eq.cummax()-1
...
def main():
    ...
    scores,vol=build_scores(px)
    full=simulate(px,scores,vol,px.index.min(),px.index.max(),80,20)
    dr=px.pct_change(fill_method=None).fillna(0)
    bench={'Strategy':full,'SPY':dr['SPY'].reindex(full.index).fillna(0),'QQQ':dr['QQQ'].reindex(full.index).fillna(0)}
    valid=[c for c in TICKERS if c in dr.columns]
    ...
    # 3y train / 1y test walk-forward, optimize only entry threshold + max positions.
    ...
    while True:
        ...
        best=None
        for e in [78,80,82,84,86]:
            for m in [10,15,20,25]:
                r=simulate(px,scores,vol,cur,min(tr_end,end),e,m)
                sh=r.mean()/r.std()*np.sqrt(252) if r.std()>0 and len(r)>30 else -999
        ...
        rows.append({'train_start':cur.date(),'train_end':min(tr_end,end).date(),'test_start':te_start.date(),'test_end':min(te_end,end).date(),'train_sharpe':best[0],'entry':best[1],'max_positions':best[2],'test_sharpe':perf(test).get('Sharpe'),'test_return':perf(test).get('TotalReturn')})
        if te_end>=end: break
    ...
    summary={'price_symbols_available':len(px.columns),'watchlist_size':len(TICKERS),'baseline_metrics':metrics.to_dict(),'oos':perf(oos),'note':'Price-only baseline. Fundamental and veteran-investor overlays are neutral/not active in this cloud run.'}
